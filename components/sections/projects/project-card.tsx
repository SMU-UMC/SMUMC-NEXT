		...
		<li className="flex flex-col gap-1 size-full max-w-xs">
			<article>
				...
				<div className="flex flex-col items-start p-3 w-full gap-1 flex-1">
					...
					<p className="font-semibold w-full text-sm truncate mb-2">
						{project.description}
					</p>

					<ul className="mt-2 flex gap-2 flex-wrap flex-1">
						{project.member.map((member) => (
							<li key={member} className="text-[11px] break-word text-zinc-400">
								{member}
							</li>
						))}
					</ul>
